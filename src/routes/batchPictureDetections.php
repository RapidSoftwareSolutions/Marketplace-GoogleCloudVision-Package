<?php
$app->post('/api/GoogleCloudVision/batchPictureDetections', function ($request, $response, $args) {
    $settings = $this->settings;

    //checking properly formed json
    $checkRequest = $this->validation;
    $validateRes = $checkRequest->validate($request, ['apiKey', 'images', 'type']);
    ...
    $body = array();
    for ($i = 0; $i < count($post_data['args']['images']); $i++) {
        $body['requests'][$i]['image']['content'] = base64_encode(file_get_contents($post_data['args']['images'][$i]));
        $body['requests'][$i]['features']['type'] = $type[$post_data['args']['type']];
        if ($post_data['args']['type'] == 'text') {
        ...
        if (!empty($post_data['args']['minCoordinates'])) {
            $body['requests'][$i]['imageContext']['latLongRect']['minLatLng']['latitude'] = doubleval(explode(',', $post_data['args']['minCoordinates'])[0]);
            $body['requests'][$i]['imageContext']['latLongRect']['minLatLng']['longitude'] = doubleval(explode(',', $post_data['args']['minCoordinates'])[1]);
        } else {
        ...
        if (!empty($post_data['args']['maxCoordinates'])) {
            $body['requests'][$i]['imageContext']['latLongRect']['maxLatLng']['latitude'] = doubleval(explode(',', $post_data['args']['maxCoordinates'])[0]);
            $body['requests'][$i]['imageContext']['latLongRect']['maxLatLng']['longitude'] = doubleval(explode(',', $post_data['args']['maxCoordinates'])[1]);
        } else {
